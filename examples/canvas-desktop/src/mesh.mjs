/**
 * The tailnet mesh bridge (P2, docs plan-truffle-desktop) — truffle/tsnet link manager that makes
 * OTHER canvas-desktop instances room members. This process's switchboard (main.mjs) stays the only
 * router; this module owns exactly the byte-moving between instances, one lane per delivery class
 * (transport.ts DeliveryClass):
 *
...
const APP_ID = "strata-collab";
const QUIC_PORT = 9430; // 443 + 9417 are truffle-reserved; QUIC listeners cannot use port 0
const UDP_PORT = 9431;
const PROTOCOL_V = 1;
