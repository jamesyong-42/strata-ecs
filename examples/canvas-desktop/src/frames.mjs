/**
 * Mesh frame codec — the hub-to-hub wire format (docs plan-truffle-desktop).
 *
 * One frame = `u32 LE headerLen | header JSON | payload bytes` — the transport-ws.ts layout with
 * `room` added (mesh links multiplex every room over one connection; a browser window's socket is
 * already room-scoped so its frames never carry one). The payload stays OPAQUE: the mesh layer reads
 * ONLY the addressing header, never the loro bytes (collab-server relay contract).
 *
...
/** header = { room, kind, from, to? } — `lane` marks a stream's first frame; `hb` the UDP heartbeat. */
