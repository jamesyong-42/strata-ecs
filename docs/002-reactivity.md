...
1. **Local-runtime only.** Reactivity observes **local runtime column changes** — nothing from Parts III–IV. It does *not* react to remote durable edits or ephemeral presence through this layer. (Those layers already compute their own change information — reconcile facts, ephemeral blob-diffs — and *could* be wired in later as additional feeds; that is explicitly out of scope here and noted as future work in §6.) This keeps the layer entirely within Part I, congruent with §0's "Part I usable on its own."
   > **Amended by 006 §C1:** "local-runtime only" holds *until Parts III–IV attach* — projection routes exclusively through the stamped primitives, so the collaborative layers feed these same stamps **by construction**. No second observer feed exists or is planned; remote edits, own-echoes, attach, and undo are observed through this layer for free. See docs/006-part3-4-amendments.md §C1 for the normative wording.
...
- **Observers do NOT survive `world.import()` / world swaps (amended).** `import()` requires an empty world, so restore replaces the World instance — and a fresh world re-mints the same dense slot/generation sequence, so a carried-over watch could *validly alias an unrelated entity*. The reactive registry belongs to its world and dies with it; apps re-subscribe after a swap (the canvas example's `onRestore` re-attach discipline, already established for the observer panel, is the pattern).
  > **Superseded (R3, IMPLEMENTED — see review-part1.md R3).** The swap pattern itself is dead: `world.reset()` / `world.import(bytes, {replace: true})` replace the document **in place, on the same World identity**, freeing every slot *with a generation bump* — so a stale handle reads dead, never aliased, and the registry survives with defined semantics: Tier-1 query watches survive and fire once at the next `notify()`; Tier-2/3 entity/value watches fire `undefined` once and self-remove (death path); resource watches survive; WorldObserver attachments survive and receive one `onReset()`. Plain `import()` into a non-empty world still throws; the bullet above remains true only for that legacy path.
- **Resource reactivity: REQUIRED fast-follow — RESOLVED by Patch Note 003 §1.** One version counter per resource, bumped in `setResource`; `observeResource(R, cb, eq?)` + `peekResource(R)`. Promoted from "natural extension" because the flagship example's hottest changing value — the `Camera` — is a resource: without this, the example's repaint logic keeps a manual `dirty.camera` flag forever, and the "reactivity dissolves the dirty flags" story stays half-told. 003 implements it as a **Tier-3-equivalent** (boxed + equality-suppressed, *not* Tier-1): `setResource` replaces the stored object wholesale, so identity alone cannot mean "changed"; the same registration-frame-boundary and `reactiveOn`-gated-stamp discipline as the entity tiers applies.
