...
- **Amended — eager cleanup closes the generation-wrap hole.** "The dead handle can never match the slot's next occupant" is only true for the first 4095 recycles (§2's documented wrap); a mounted React subscription is exactly the long-lived handle the wrap defense assumed away, and a wrapped re-issue would make a stale watch silently fire with a *different entity's* value. So the reactive layer additionally registers **one internal lifecycle observer** (the shipped T0 `world.observe` `onDestroy` hook — its first non-tools consumer) when the first entity/value watcher registers: on destroy of a watched entity, the watch fires `undefined` and is removed **at that pass**, not lazily. The `get`→`undefined` path remains as the belt-and-braces fallback; with eager cleanup, no watch survives long enough to be ABA'd.
...
- **Stable references.** `peek` returns Tier 3's value box; `queryResult` returns a cached array whose identity changes only when membership or a watched column changed. This satisfies `useSyncExternalStore`'s hard requirement — a snapshot that returns a fresh reference every call causes an infinite render loop. The tiers already store the previous state, so the stable reference falls out for free.
...
| Part I API reference (near `read`/`get`, § line 1042) | Add the `Reactive` interface (`observeQuery`/`observeEntity`/`observeValue` + `peek`/`queryResult`) and `world.reactive`. |
