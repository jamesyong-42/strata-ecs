...
> **As-built addendum (2026-07-04, PART IV complete — commits a024402/dab91b9/a0289b3/9c4dd09/889c4f9; the shipped forms).**
> 1. **Construction:** `createEphemeralStore(new LoroEphemeralSnapshot(loroEphemeralStore), { peerId, send, throttleMs?, ttlMs? })` — the caller builds the adapter (it is the ONE public adapter class; `createDurableStore(loroDoc)` hides its adapter, ephemeral does not, because the store is written against the `EphemeralSource` interface). `attachEphemeral(world, eph)` is a package-level function (the durable Decision-A rationale). `Local`'s canonical home is the CORE barrel.
> 2. **Adapter realities that supersede §15.3/§15.5's sketches** (spike-verified, loro-crdt 1.13.6): events carry key lists (values read at the source); the keepalive **re-sets** own-MINTED keys (B5) and **skips currently-dirty keys** (they ride the throttle with a fresh stamp — the same-ms tie is otherwise real); `encodePartition(prefix)` became `encodeKeys(keys)` (a prefix scan cannot separate minted keys from learned ghosts); deletes cross the wire ONLY as individual tombstones and a same-ms delete-vs-set can drop — **TTL is the despawn guarantee**, `leave()` merely shortens latency; same-ms LWW ties are first-applied-wins, which is why the ≥1ms send spacing and writer partitioning are correctness-critical, not tuning.
> 3. **The hostile-inbound discipline (the P4 review's north star):** every inbound reject is SOFT (warn-once + skip) — malformed values, unknown names, and the inbound eid-field ban never throw out of `world.sync()`; framework-reserved tags (`Local`, per `isReservedName`) are **wire-proof** — a forged blob cannot apply them to a projection; key parsing is anchored to the minted `${peerId}-<integer>` shape (hostile keys cannot skew `peerCount` or suppress a legitimate peer). A fully-rejected unseen blob is still cached so TTL cleans any residue.
> 4. **Lifecycle adjudications:** detach uses the per-listener unsubscribe and **purges the adapter's outbound staging** (deletes minted keys locally + drains tombstones WITHOUT broadcasting — detach ≠ leave; otherwise a re-attach re-broadcasts a despawned entity as a ghost cursor); same-store re-attach is a supported lifecycle; the adapter's `dispose()` is for final app teardown only. The own-timeout revive path is bounded (once per ttl per key, one-shot warn) and doubles as a ttl-cadence backup keepalive — no backoff needed. `EphemeralSyncStatus` = `{ peerCount, lastInboundFrame }`, set-on-change, published at drain end (no pendingInbound field → no enqueue publish), removed at detach.
> 5. **Demo-surfaced transport constraints (a8ba1b9's collab demo, normative for any transport):** `DurableStore.exportSnapshot(): Uint8Array` exists for joiner bootstrap (§14.2 wiring). **Bootstrap is BIDIRECTIONAL:** a joiner must broadcast its own snapshot after importing the responder's — its first increment references its own construction commit, which existing peers lack (they would quarantine). Increments arriving before a peer's bootstrap completes must be app-buffered, never fed to `applyRemote` first. Known leaderless-bootstrap limits (accepted): simultaneous cold-join can split-seed; the pre-bootstrap buffer is bounded only by the bootstrap window.
